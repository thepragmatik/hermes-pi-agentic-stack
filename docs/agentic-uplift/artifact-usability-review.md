...
| LCM + Mnemosyne design/configs | local context/memory qualification contract | installed/benchmarked production memory | research/config-ready; canary P0 |
...
9. Qualify the selected **local context/memory profile** against the built-in control: exact recovery, cross-session recall precision/staleness, tool/context token overhead, restart/backup, memory poisoning, storage/RSS growth, and successful steady-state operation with outbound network denied. Rehearse independent rollback of the context engine and external memory provider.
...
- The initial memory recommendation treated built-in memory as the long-term default and underweighted the user's previously successful LCM + Mnemosyne pattern. The refined design now separates **context-engine responsibility from durable-memory responsibility** and requires a four-profile benchmark rather than choosing from feature lists.
- LCM v0.20 includes optional semantic/cross-session recall and Mnemosyne can automatically persist/inject/consolidate memory. Enabling all features in both would duplicate state and context. The playbook now disables overlapping automation initially and gives each store one owner role.
- Mnemosyne's effective configuration defaults have changed over time and some documented/generated defaults differ from implementation fallbacks. The playbook therefore pins releases, explicitly sets conservative canary behavior, and treats upstream defaults as untrusted until runtime status/config evidence is captured.
