...
PUBLIC_FILES=[
 ...
 'docs/agentic-uplift/README.md','docs/agentic-uplift/SOURCES.md','docs/agentic-uplift/architecture.md','docs/agentic-uplift/architecture.graph.json',
 'docs/agentic-uplift/agent-execution-contract.md','docs/agentic-uplift/artifact-usability-review.md','docs/agentic-uplift/adversarial-review.md','docs/agentic-uplift/validation-report.md','docs/agentic-uplift/site-publishing.md','docs/agentic-uplift/implementation-playbook.md','docs/agentic-uplift/spec-kit-profiles.md','docs/agentic-uplift/savings-model.md',
 'docs/agentic-uplift/research/local-routing-models.md','docs/agentic-uplift/research/context-token-optimization.md','docs/agentic-uplift/research/hermes-pi-lsp.md','docs/agentic-uplift/research/security-zero-trust-pii.md','docs/agentic-uplift/research/skill-slimming-slicing.md',
 'skills/hermes-stack-uplift/SKILL.md',
 'skills/hermes-stack-uplift/references/00-preflight.md','skills/hermes-stack-uplift/references/10-baseline-and-backup.md','skills/hermes-stack-uplift/references/20-context-and-skills.md','skills/hermes-stack-uplift/references/30-router.md','skills/hermes-stack-uplift/references/40-security-and-policy.md','skills/hermes-stack-uplift/references/50-pi-and-lsp.md','skills/hermes-stack-uplift/references/60-evaluation-and-promotion.md','skills/hermes-stack-uplift/references/70-upgrades-and-rollback.md']

def rewrite_href(href: str, page: str) -> str:
    if href.startswith(('http://','https://','#','mailto:')): return href
    base,*frag=href.split('#',1); clean=base.rstrip('/')
    if clean in PUBLIC_FILES:
        target=rel(page,'raw/'+clean); return target+(('#'+frag[0]) if frag else '')
    return 'https://github.com/thepragmatik/hermes-pi-agentic-stack/blob/main/'+href

def render_inline(text: str,page: str)->str:
    x=html.escape(text); x=re.sub(r'`([^`]+)`',r'<code>\1</code>',x)
    def repl(m):
        label=m.group(1); href=html.unescape(m.group(2)); return f'<a href="{html.escape(rewrite_href(href,page),quote=True)}">{label}</a>'
    x=re.sub(r'\[([^\]]+)\]\(([^)]+)\)',repl,x); return re.sub(r'\*\*([^*]+)\*\*',r'<strong>\1</strong>',x)

def render_md(text: str,page: str)->str:
    out=[]; in_code=False; code=[]; list_open=False
    for line in text.splitlines():
        ...
        if line.startswith('#'):
            if list_open: out.append('</ul>'); list_open=False
            n=min(6,len(line)-len(line.lstrip('#'))); out.append(f'<h{n}>'+render_inline(line[n:].strip(),page)+f'</h{n}>'); continue
        if line.startswith('- '):
            if not list_open: out.append('<ul>'); list_open=True
            out.append('<li>'+render_inline(line[2:].strip(),page)+'</li>'); continue
        if line.startswith('> '):
            if list_open: out.append('</ul>'); list_open=False
            out.append('<blockquote>'+render_inline(line[2:].strip(),page)+'</blockquote>'); continue
        if line.startswith('|'):
            if list_open: out.append('</ul>'); list_open=False
            out.append('<pre>'+html.escape(line)+'</pre>'); continue
        out.append('<p>'+render_inline(line,page)+'</p>')
    if in_code: out.append('<pre><code>'+html.escape('\n'.join(code))+'</code></pre>')
    if list_open: out.append('</ul>')
    return '\n'.join(out)

def rel(from_page,target): return '../'*len(Path(from_page).parent.parts)+target

def layout(title,body,page,raw):
    nav=' '.join(f'<a href="{rel(page,p)}">{html.escape(t)}</a>' for p,(t,_) in list(PAGES.items())[:9])
...
def main():
    ...
    out.mkdir(parents=True); (out/'raw').mkdir(); manifest=[]
    for src in PUBLIC_FILES:
        p=ROOT/src
        if not p.exists(): raise SystemExit(f'missing public source: {src}')
        dst=out/'raw'/src; dst.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(p,dst); manifest.append({'path':src,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'bytes':p.stat().st_size})
